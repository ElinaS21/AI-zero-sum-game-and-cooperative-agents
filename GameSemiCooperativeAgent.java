...
public class GameSemiCooperativeAgent extends GameAgent {
	...
	void sortChildren(LinkedList<StateNode> children, StateNode parent) {
		...
		if(parent.depth % 2 == 0)
			parent.children.sort(semiStateNodeComparatorMinToMax);
		else
			parent.children.sort(semiStateNodeComparatorMinToMaxOppH);
	...
	/*heuristic =  from max of(agent heuristic) choose max of(opponent heuristic)
	 * help opponent just if it not impact on our score*/
